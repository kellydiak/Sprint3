...
class EntraineurDAO(val bdd: BDD = db) {
    ...
    fun findAll(): MutableList<Entraineur> {
        ...
        val sql = "SELECT * FROM Entraineur"
    ...
    fun findById(id: Int): Entraineur? {
        ...
        val sql = "SELECT * FROM Entraineur WHERE id = ?"
        val requetePreparer = bdd.connectionBDD!!.prepareStatement(sql)
        requetePreparer.setInt(1, id) // insère la valeur de l'id dans la requete preparer
        val resultatRequete = bdd.executePreparedStatement(requetePreparer)
    ...
    fun findByNom(nomRechercher: String): MutableList<Entraineur> {
        val result = mutableListOf<Entraineur>()
        val sql = "SELECT * FROM Entraineur WHERE nom = ?"
        val requetePreparer = bdd.connectionBDD!!.prepareStatement(sql)
    ...
    fun save(entraineur: Entraineur): Entraineur? {
        ...
        if (entraineur.id == 0) {
            // Insertion
            val sql = "INSERT INTO Entraineur (nom, argents) VALUES (?, ?)"
            ...
            // Mise à jour
            val sql = "UPDATE Entraineur SET nom = ?, argents = ? WHERE id = ?"
    ...
    */
    ...
    fun deleteById(id: Int): Boolean {
        val sql = "DELETE FROM Entraineur WHERE id = ?"
        val requetePreparer = bdd.connectionBDD!!.prepareStatement(sql)
